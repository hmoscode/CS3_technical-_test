...
export class Init1757441685095 implements MigrationInterface {
    name = 'Init1757441685095'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`invoice_details\` (\`id\` bigint NOT NULL AUTO_INCREMENT, \`invoiceId\` bigint NOT NULL, \`articleId\` bigint NOT NULL, \`quantity\` int NOT NULL, \`unitPrice\` decimal(10,3) NOT NULL, \`totalPrice\` decimal(10,3) NOT NULL, \`priceWithWholesale\` decimal(10,3) NULL, \`createdAt\` timestamp(6) NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` timestamp(6) NULL DEFAULT CURRENT_TIMESTAMP(6), \`deletedAt\` timestamp(6) NULL DEFAULT CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
